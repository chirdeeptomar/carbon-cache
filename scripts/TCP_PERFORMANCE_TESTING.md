...
2. **Start the server**:
```bash
# Option 1: Unified server (HTTP + TCP)
cargo run --release --bin server-unified
...
**Solutions**:
- Use release build: `cargo run --release --bin server-unified`
...
- [Unified Server Architecture](../server-unified/)
